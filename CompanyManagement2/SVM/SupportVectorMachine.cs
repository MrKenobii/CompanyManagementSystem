...
public class SupportVectorMachine
{
    ...
    public double ComputeDecision(double[] input)
    {
        ...
        for (int i = 0; i < this.supportVectors.Count; ++i)
        {
            ...
            {
                sum += this.weights[i] * this.PolyKernel(this.supportVectors[i], input);
            } 
            else if(this.kernelType == "rbf")
            {
                sum += this.weights[i] * this.RbfKernel(this.supportVectors[i], input);
            }
